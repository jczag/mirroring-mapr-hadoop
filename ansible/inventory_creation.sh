...
ssh-keygen -t rsa -b 4096 -f ~/.ssh/id_rsa -N ""  # Nouvelle clé SSH sans password 
START=1
END=3
SPECIAL_IP="192.168.10.4"  Adresse IP spéciale à vérifier
SUBNET="192.168.10"
USERNAME="ansible"
PASSWORD="ansible"
...
# check connectivité des machines
...
for IP in $(seq $START $END) $SPECIAL_IP; do
    ...
    if ping -c 1 $FULL_IP &> /dev/null; then
        ...
        # Copie de la clé SSH vers le hôte distant
        ...
        #Vérification d'existance de la clé pour éviter les doublons
